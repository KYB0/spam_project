...
<body>
    <form method="post" action="/spam/reservation">
        <!-- 예약을 서버로 전송하는 폼 추가 -->
    ...
    <script>
        ...
        $(function () {
            ...
            function generateTimeSlots(selectedDate, type) {
                ...
                timeSlotContainer.empty(); // 이전에 생성된 목록을 삭제

                while (startTime < endTime) {
                    // 현재 시간 이후의 시간만 목록으로 생성
                    if (startTime > currentTime) {
                        let timeSlotLi = $("<li>").text(startTime.toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit'
                        })).data("startTime", new Date(startTime)); // 시작 시간을 목록 데이터로 저장

                        // timeSlotLi.click(function () {
                        //     // 목록을 클릭했을 때 선택한 시작 시간을 가져옴
                        ...
                        timeSlotContainer.append(timeSlotLi);
                    }
