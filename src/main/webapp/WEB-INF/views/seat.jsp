                    ...
                    document.querySelector(".myChooseButton").addEventListener("click", function () {
                        if (selectedSeat !== null) {
                            ...
                            $.ajax({
                                ...
                                data: JSON.stringify({ room_id, seat_number }),
                                contentType: "application/json",
                                success: function (response) {
                                    console.log("예약 세부 정보:", response);
            
                                    // 서버 응답에서 예약된 시간 정보를 파싱하여 reservedTimes 배열에 추가
                                    ...
                                    // 예약된 시간 슬롯 비활성화
                                    reservedTimes = response; // 서버 응답에서 바로 예약된 시간 목록을 가져옵니다.
                                    ...
                                    // 예약된 시간 슬롯 비활성화
                                    reservedTimes.forEach(function (reservation) {
                                        const startTime = moment(reservation.start_time);
                                        const endTime = moment(reservation.end_time);
            
                                        // 선택한 날짜와 예약된 시간 사이에 겹치는 경우 해당 시간을 비활성화
                                        if (selectedDate.isSameOrBefore(endTime) && selectedDate.isSameOrAfter(startTime)) {
                                            const startHour = startTime.format("HH:mm");
                                            const endHour = endTime.format("HH:mm");
            
                                            // 시간 슬롯을 순회하면서 예약된 시간과 비교
                                            $(".time-list li").each(function () {
                                                const time = $(this).text().split(':')[0] + ':00'; // 시간 형식을 맞춰줍니다.
                                                if (time >= startHour && time < endHour) {
                                                    $(this).prop('disabled', true).addClass('reserved-time');
                                                }
                                            });
                                        }
                                    });
